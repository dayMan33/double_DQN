...
Transition = namedtuple('Transition', ('state', 'action', 'reward', 'next_state', 'legal_actions', 'terminal'))


class ExperienceReplay:
    def __init__(self, e_max=15000):
    ...
    def get_batch(self, batch_size: int):
        ...
        terminal = np.array(batch.terminal)
        return state, action, next_state, reward, legal_actions, terminal

    def add(self, state, action, reward, next_state, legal_actions, is_terminal):
        """remove the oldest experience if the memory is full"""
        if self.get_num() > self.get_max():
            del self.transitions[0]
        """add single experience"""
        self.transitions.append(Transition(state, action, reward, next_state, legal_actions, is_terminal))
